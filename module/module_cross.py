...
# Last Change:  2023-12-16 12:00:01
...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, query, key, value, kv_mask, attn_mask=None, need_weights=False):
        ...
        kv_mask和attn_mask区别：
            ...
            2）cross-attention. 如果attn_mask是None，表示q全部参与attention，只需要kv_mask. 如果attn_mask不是None，表示q不能全部可见，如翻译的时候。
        '''
...
class CrossModel(PreTrainedModel):
    ...
    def forward(self, q, kv, q_mask, kv_mask):
        ...
        '''
        if q_mask is None:
            q_mask = torch.ones(q.size(1), kv.size(1))
        if kv_mask is None:
            kv_mask = torch.ones(kv.size(0), kv.size(1))
