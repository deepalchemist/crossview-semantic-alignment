...
# Last Change:  2023-09-12 07:03:12
"""
...
class VisualTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, video_frame=-1, attn_mask=None):
        # x (b 3 H W) video_frame=b=bs*max_frame
        if self.linear_patch == '3d':
            ...
            x = self.conv1(x)  # shape = [b, d, grid, grid] e.g., [b 768 7 7]

        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, d, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, d]
        x = torch.cat([self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device), x], dim=1)  # shape = [*, grid ** 2 + 1, d]
        x = x + self.positional_embedding.to(x.dtype)
        x = self.ln_pre(x)

        x = x.permute(1, 0, 2)  # NLD -> LND
        x = self.transformer(x, video_frame=video_frame, attn_mask=attn_mask)
...
class CLIP(nn.Module):
    ...
    def get_config(pretrained_clip_name="ViT-B/32"):
        model_path = "./pretrained/ViT-B-32.pt"
        if pretrained_clip_name in _MODELS and pretrained_clip_name in _PT_NAME:
            ...
            model_path = f"./pretrained/{_PT_NAME[pretrained_clip_name]}"
    ...
    def encode_image(self, image, return_hidden=False, video_frame=-1, attn_mask=None):
        hidden = self.visual(image.type(self.dtype), video_frame=video_frame, attn_mask=attn_mask)
    ...
    def encode_text(self, text, return_hidden=False):
        ...
        x = x + pos_emd
        x = x.permute(1, 0, 2)  # bs n_word d -> n_word bs d
        x = self.transformer(x)
        x = x.permute(1, 0, 2)  # n_word bs d -> bs n_word d
