...
# Last Change:  2023-12-16 11:15:05
...
import math
from module.util_config import PretrainedConfig
...
def genPatchmask(box_lst, grids, iou_thresh=0.5, cls_token=True):
    num_grid = len(grids)
    grid_iou = [[IOU(_box, t)>=iou_thresh for _box in box_lst] for t in grids]
    grid_mask = [int(any(t)) for t in grid_iou]
    if sum(grid_mask)==0:
        grid_mask = [1] * len(grid_mask)
    if cls_token:
...
class ReconsWeight(nn.Module):
    def __init__(self, in_dim):
        super(ReconsWeight, self).__init__()
        #self.conv = nn.Sequential(
        #    nn.Conv2d(in_dim, in_dim*8, kernel_size=(1, 1)),
        #    nn.ReLU(inplace=True),
        #    nn.Conv2d(in_dim*8, 1, kernel_size=(1, 1)),
        #    nn.ReLU(inplace=True),
        #)
        ...
        self.conv = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(in_dim//2, 1, kernel_size=(1, 1)),
            nn.ReLU(inplace=True),
        )
    ...
    def forward(self, attn_weights):
        """
        Inputs:
            attn_weights: (bs num_heads N M)
        Outputs:
            (bs 1 N M)
        """
        return self.conv(attn_weights)
...
class PreTrainedModel(nn.Module):
    ...
    @classmethod
    def load_preweight(cls, model, state_dict, local_rank, prefix=None):
        old_keys = []
        new_keys = []
        ...
        if prefix is None and local_rank == 0:
            logger.info("-" * 20)
            if len(missing_keys) > 0:
                logger.info("The following weights of {} not initialized from pretrained model: {}"
                            .format(model.__class__.__name__, "\n   " + "\n   ".join(missing_keys)))
            if len(unexpected_keys) > 0:
                logger.info("The following weights from pretrained model not used in {}: {}"
                            .format(model.__class__.__name__, "\n   " + "\n   ".join(unexpected_keys)))
            if len(error_msgs) > 0:
                logger.error("The following weights from pretrained model cause errors in {}: {}"
                             .format(model.__class__.__name__, "\n   " + "\n   ".join(error_msgs)))
...
class CrossEn(nn.Module):
    ...
    def forward(self, sim_matrix, diag=True):
