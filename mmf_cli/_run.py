...
# Last Change:  2024-05-01 08:23:36
import argparse
import logging
import random
import typing
...
def distributed_main(device_id, configuration, predict=False):
    ...
    logger.info(f"Current device id: {device_id}")
...
def run(opts: typing.Optional[typing.List[str]] = None, predict: bool = False):
    ...
    if config.distributed.init_method is None:  # True
        infer_init_method(config)  # Do nothing when single_node training, multi-node multi gpu entrance
    ...
    if config.distributed.init_method is not None:
        ...
        if torch.cuda.device_count() > 1 and not config.distributed.no_spawn:
            ...
            torch.multiprocessing.spawn(
                fn=distributed_main,
                args=(configuration, predict),  # (i, *), i is index of nprocs
                nprocs=torch.cuda.device_count(),
            )
        else:
            distributed_main(0, configuration, predict)
    elif config.distributed.world_size > 1:
        if is_xla():  # False if training.device=cuda
            ...
            logger.info(">>> Single-Node Multi-GPU Training Forwarding")
