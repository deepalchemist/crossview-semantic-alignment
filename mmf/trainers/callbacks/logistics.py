...
class LogisticsCallback(Callback):
    ...
    def on_update_end(self, **kwargs):
        ...
        _module = self.trainer.model.module if hasattr(self.trainer.model, "module") else self.trainer.model
        working_task = getattr(_module, "working_task", "error")
        ...
        unique_lr = list(set(
                [t['lr'] for t in self.trainer.optimizer.param_groups]
        ))
        unique_lr.sort()
        unique_lr = "&".join([f"{t:.5e}" for t in unique_lr])

        #name2lr = [(t["names"][0], t['lr']) for t in self.trainer.optimizer.param_groups]
        #df = open("name_to_lr.txt", "w")
        #[df.write(f"{t[0]}\t{t[1]}\n") for t in name2lr]
        #df.close()
