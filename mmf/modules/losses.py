...
"""
# Last Change:  2024-04-29 13:03:44
...
@registry.register_loss("hard_negative_cross_entropy")
class HardNegativeCrossEntropyLoss(nn.Module):
    def __init__(self,):
        super().__init__()

    def forward(self, sample_list, model_output):
        sim_matrix = model_output["scores"]
        _, index = sim_matrix.max(dim=-1)
        gt = torch.zeros(len(index), dtype=index.dtype, device=index.device)
        acc = (index==gt).sum() / len(gt) * 100.
        logger.info(f"Accuracy of cross model: {acc:.2f}%")

        diag = model_output["diag"]
...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, model_output: Dict[str, Tensor]):
