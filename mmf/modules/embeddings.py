...
# Last Change:  2024-05-08 14:55:14
...
class VideoImageEmbeddings(nn.Module):
    def __init__(self, config):
        ...
        # Two type of image and video, 0 for image and 1 for video
        self.image_token_type = 0
        self.video_token_type = 1
        self.token_type_embedding = nn.Embedding(2, embed_dim)

        self.image_position_embedding = nn.Embedding(config.max_image_position, embed_dim)
        self.video_position_embedding = nn.Embedding(config.max_video_position, embed_dim)
        self.dropout = nn.Dropout(0.1)

        # position_ids (1, len position emb) is contiguous in memory and exported when serialized
        self.register_buffer(
            "image_position_ids", torch.arange(config.max_image_position).expand((1, -1)), persistent=False
        )  # [1 max_image_position]
        
        self.register_buffer(
            "video_position_ids", torch.arange(config.max_video_position).expand((1, -1)), persistent=False
        )  # [1 max_video_position]
    ...
    def encode_image(
        ...
        class_embeds = self.class_embedding.expand(bsz, 1, -1)  # [bsz 1 d]
        image_embeds = torch.cat([class_embeds, image_embeds], dim=1)
        l = image_embeds.size(1)
        token_type_ids = torch.zeros(
                (bsz, l),
                device=image_embeds.device
                ).long() + self.image_token_type

        token_type_embeds = self.token_type_embedding(token_type_ids)  # [bsz l d]

        position_ids = self.image_position_ids[:, :l].expand(bsz, -1)  # [bsz l]
        position_embeds = self.image_position_embedding(position_ids)
        embeddings = (
                image_embeds 
                + token_type_embeds 
                + position_embeds
    ...
    def encode_video(
        ...
        bsz, l, _ = video_embeds.shape
        token_type_ids = torch.zeros(
                (bsz, l),
                device=video_embeds.device
                ).long() + self.video_token_type

        token_type_embeds = self.token_type_embedding(token_type_ids)  # [bsz l d]

        position_ids = self.video_position_ids[:, :l].expand(bsz, -1)  # [bsz l]
        position_embeds = self.video_position_embedding(position_ids)
    ...
    def forward(
        ...
        video_embeds = self.encode_video(video_embeds)
       
        embeddings = torch.cat([image_embeds, video_embeds], dim=1)
