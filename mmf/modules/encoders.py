...
import importlib
# Last Change:  2024-05-08 14:54:56
import logging
import math
import os
import pickle
...
class ImageEncoderFactory(EncoderFactory):
    ...
    def __init__(self, config: Config, *args, **kwargs):
        ...
        if self._type == "default" or self._type == "identity":
            self.module = nn.Identity()
            self.module.out_dim = params.in_dim
        elif self._type == "huggingface_clip":
            self.module = CLIPEncoder(params)
        elif self._type == "openai_clip":
            self.module = OpenAICLIPEncoder(params)
...
@registry.register_encoder("huggingface_clip")
class CLIPEncoder(Encoder):
    @dataclass
    class Config(Encoder.Config):
        # See https://huggingface.co/openai/clip-vit-base-patch32 for available opts
        name: str = "clip"
        pretrained_model_name: str = "pretrained/clip-vit-base-patch32"
    ...
    def __init__(self, config: Config, *args, **kwargs):
        ...
        clip = CLIPModel.from_pretrained(config.pretrained_model_name)
        # Model config: huggingface/transformers/src/transformers/models/clip/modeling_clip.py
        self.model = clip.vision_model
        self.visual_projection = clip.visual_projection
        #processor = CLIPProcessor.from_pretrained(config.pretrained_model_name)
        clip_config = clip.config
        embed_dim = clip_config.vision_config.hidden_size
        projection_dim = clip_config.projection_dim

        ckpt_path = getattr(config, "pretrained_model_path", None)
        if ckpt_path is not None:
            state_dict = torch.load(ckpt_path, map_location="cpu")
            fmt_state_dict = OrderedDict()
            for _key, _val in state_dict.items():
                if not "image_encoder" in _key: 
                    continue
                if _key.startswith("module."):
                    _key = _key.replace("module.", "")
                _key = _key.replace("image_encoder.", "")
                fmt_state_dict[_key] = _val
 
            self.load_state_dict(fmt_state_dict, strict=True)
            print(f"Successfully load image encoder from: {ckpt_path}")

        #self.patch_layernorm = nn.LayerNorm(embed_dim, eps=clip_config.vision_config.layer_norm_eps)
        #self.patch_projection = nn.Linear(embed_dim, projection_dim, bias=False)
        #self.initialize_patch_weight_from_clip(clip)
 
    def initialize_patch_weight_from_clip(self, clip):
        self.patch_layernorm.weight = nn.Parameter(
                deepcopy(clip.vision_model.post_layernorm.weight.data), requires_grad=True
        )
        self.patch_projection.weight = nn.Parameter(
            deepcopy(clip.visual_projection.weight.data), requires_grad=True
        )
        return
    ...
    def forward(self, pixel_values, 
        ...
        # last_hidden_state shape = [bs seq_len d]
        # pooler_output shape = [bs d]
        pooled_output = vision_outputs[1]  #  after LN 
        image_features = self.visual_projection(pooled_output)

        #patch_features = vision_outputs[0][:, 1:, :]  # w/o LN and project
        ## Follow https://github.com/huggingface/transformers/blob/9fe3f585bb4ea29f209dc705d269fbe292e1128f/src/transformers/models/clip/modeling_clip.py#L859
        #patch_features = self.patch_layernorm(patch_features)
        ## Follow https://github.com/huggingface/transformers/blob/9fe3f585bb4ea29f209dc705d269fbe292e1128f/src/transformers/models/clip/modeling_clip.py#L1125
        #patch_features = self.patch_projection(patch_features)
        #patch_features = torch.cat([image_features.unsqueeze(1), patch_features], dim=1)

        last_hidden_state = vision_outputs[0]
        #last_hidden_state = self.model.post_layernorm(last_hidden_state)
        #last_hidden_state = self.visual_projection(last_hidden_state)
        #image_features = last_hidden_state[:, 0, :]
        #patch_features = last_hidden_state[:, 1:, :].contiguous()
        patch_features = last_hidden_state  # before LN
        return image_features, patch_features

@registry.register_encoder("openai_clip")
class OpenAICLIPEncoder(Encoder):
    @dataclass
    class Config(Encoder.Config):
        # See https://huggingface.co/openai/clip-vit-base-patch32 for available opts
        name: str = "clip"
        pretrained_model_name: str = "pretrained/ViT-B-32.pt"
    ...
    def forward(
        ...
        # last_hidden_state shape = [bs seq_len d]
        # pooled_output shape = [bs d]
        #image_features = pooled_output  # w/ post_ln and proj
        patch_features = last_hidden_state  # w/o post_ln and proj
        #patch_features = self.model.ln_post(last_hidden_state) @ self.model.proj
        image_features = patch_features[:, 0, :]
        image_features = self.model.ln_post(image_features) @ self.model.proj
