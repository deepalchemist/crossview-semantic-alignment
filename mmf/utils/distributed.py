...
# Last Change:  2024-05-01 08:02:16
import contextlib
import logging
import os
import pickle
...
def infer_init_method(config):
    ...
    registry.register("is_xla", config.training.get("device", "cuda") == "xla")  # False

    # support torch.distributed.launch
    # NOTE: multi-node multi-gpu entrance
    if all(
        key in os.environ
        ...
        config.distributed.no_spawn = False  # FIXME: True or False
        logger.info(
            f"Getting Distributed Config from os.environ: "
            f"init_method={config.distributed.init_method} "
            f"world_size={config.distributed.world_size} "
            f"rank={config.distributed.rank} "
            f"no_spawn={config.distributed.no_spawn}"
        )
...
def distributed_init(config):
    ...
    logger.info(f"XLA Mode:{is_xla()}")  # False
    ...
    if is_xla():
        ...
        logger.info(
            f"Setting Distributed Params for Each local_rank: "
            f"backend={config.distributed.backend} "
            f"init_method={config.distributed.init_method} "
            f"world_size={config.distributed.world_size} "
            f"rank={config.distributed.rank}"
