...
# Last Change:  2024-04-24 03:48:48
import copy
import random
...
# version 4.10.1
# When network connection error, download from https://huggingface.co/google-bert/bert-base-uncased
...
class MaskedTokenProcessor(BaseProcessor):
    ...
    def __init__(self, config, *args, **kwargs):
        ...
        self._tokenizer = AutoTokenizer.from_pretrained(
            #tokenizer_config.type, **tokenizer_config.params  # TODO(fixed)
            tokenizer_config.path, **tokenizer_config.params
        )
...
class FashionViLTextTokenizer(MaskedTokenProcessor):
    def __init__(self, config, *args, **kwargs):
        ...
        #from_pretrained_name = config.get("from_pretrained", "bert-base-uncased")  # TODO(fixed)
        #kwargs_dict = dict(kwargs, do_lower_case="uncased" in from_pretrained_name)
        from_pretrained_name = config.tokenizer_config.type
        kwargs_dict = dict(kwargs, **config.tokenizer_config.params)
        self._tokenizer = BertTokenizer.from_pretrained(
            from_pretrained_name, **kwargs_dict
        )
        self._max_seq_length = config.get("max_seq_length", 25)
