...
def parsing_args():
    ...
    parser.add_argument('-c', '--conditional', type=str, default=None, help='path of conditional     query videos')
...
def predict_topk(config, output_root, cdt_video):
    ...
    if cdt_video is not None:
        ...
        pred = {k:v for k,v in pred.items() if k in cdt_video}
...
def init_model(config):
    ...
    #ckpt_path = os.path.join(config.env.save_dir, f"iter4000.ckpt")
...
def extract_gallery(config, output_root, device_id):
    ...
    for sample in tqdm(gallery_loader, desc="gallery forward"):
        ...
        assert len(liveid_lst)==len(imageid_lst)==img_embs.size(0)
        for _liveid, _imageid, _emb in zip(liveid_lst, imageid_lst, img_embs):
            if _liveid not in l2g:
                l2g[_liveid] = {"name": list(), "emb": list()}
            l2g[_liveid]["name"].append(_imageid)
            l2g[_liveid]["emb"].append(_emb)

    for _liveid in l2g.keys():
        l2g[_liveid]["emb"] = torch.stack(l2g[_liveid]["emb"], dim=0)
...
@torch.no_grad()
def extract_query(config, output_root, device_id, cdt_video):
    BATCH_SIZE = 32
    device = torch.device(f"cuda:{device_id}")
    ...
    for sample in tqdm(query_loader, desc=f"query forward"):
        ...
        assert len(liveid_lst)==len(videoid_lst)==video_embs.size(0)

        for _liveid, _videoid, q_f in zip(liveid_lst, videoid_lst, video_embs):
            if cdt_video is not None and _videoid not in cdt_video:
                continue
            g_f = live2g[_liveid]["emb"]  # gallery image feature, [m d]
            ...
            item_onshelf = live2g[_liveid]["name"]
...
if __name__ == "__main__":
    ...
    if args.conditional is not None:
        cdt_path = os.path.join(config.dataset_config.videotoshop.data_dir, "annotations", args.conditional)
        cdt_video = [t.strip().split("\t")[0] for t in open(cdt_path).readlines()]
    ...
    extract_query(config, output_root, args.device_id, cdt_video)
    
    # Step3: Topk accuracy
    predict_topk(config, output_root, cdt_video)
